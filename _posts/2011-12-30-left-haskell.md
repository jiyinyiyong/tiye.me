...
---
	...
	Jekyll 过于复杂, 结果, 不小心把过去练手的大量代码弄没了
	Google 只索引到了下面这份 <http://goo.gl/KJuUp>
	其它的发部分是 js Demo, 价值的确不大, 但毕竟花过时间
	当前能会想到的有下面这些
	+ Genie 笔记, 对官网的内容基本抄了一遍, 中文资料稀少啊
	+ js 图形, 土豆剩视频 <http://www.tudou.com/programs/view/QnHu0rCIL3k/>
	+ 一份初学 js 时的摘记, 不重要, nodebeginner 中文类似
	+ 挑花标记的字, 原来尝试做了输入法, 只剩截图 <http://jiyinyiyong.yupoo.com/>
	+ 双三 coffee 写的算法片段, Python 还在, 级别低, 不疼
	+ 若干想不起来..
	时间期末, 我复习不进去教材, 看文档的能力暴涨, 几天都玩
	也许是一种报应了, 还是说暗示说我应该专心 hs 和 node? :P
	日记存另一个仓库, 还没折腾, 所以没丢, 晚上折腾
	第一次事故
