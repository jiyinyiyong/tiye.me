            ...
            defcomp comp-card (idx key on-open on-close)
              [] (effect-fading)
                let
                    ...
                    title $ info .and-then
                      fn (page) (get page :title)
                    content $ info .and-then
                      fn (page) (get page :content)
                    title-text $ title .unwrap-or "|Unknown page"
                    directives $ content .unwrap-or ([])
                  div
                    {} (:class-name css-card)
                      :style $ {}
        ...
        'comp-container $ %{} 'CodeEntry (:doc |)
          :code $ quote
            defcomp comp-container (reel)
              let
                  store-option $ get reel :store
                  store $ store-option .unwrap-or ({})
                  states-option $ get store :states
                  states $ states-option .unwrap-or ({})
                  router-option $ get store :router
                  router $ router-option .unwrap-or ([])
                  push-tab $ fn (idx x d!)
                    d! $ :: :push-page idx x
        ...
        'effect-bump $ %{} 'CodeEntry (:doc |)
          :code $ quote
            defeffect effect-bump () (action el at?)
              let
                  element $ browser-object el
                  style $ browser-object (.-style element)
                  action-tag $ unsafe-coerce action 'Tag
                if (= action-tag :mount)
                  do
                    -> style .-scale $ set! 0.6
                    js/setTimeout $ fn ()
            ...
            defeffect effect-fading () (action el at?)
              let
                ...
                do
                  if (= action-tag :mount)
                    do
                      -> style .-opacity $ set! 0.1
                      -> style .-transform $ set! "|scale(0.6) translate(-80px,0px)"
                      flipped js/setTimeout 0 $ fn ()
                        -> style .-opacity $ set! 1
                        -> style .-transform $ set! "|scale(1) translate(0,0)"
                      flipped js/setTimeout 240 $ fn ()
                        let
                            corner $ browser-object (.!querySelector element |.right-corner)
                          .!scrollIntoView corner $ js-object (:block |end) (:behavior |smooth)
                  when (= action-tag :unmount)
                    let
                        el' $ browser-object (.!cloneNode element)
                        parent $ browser-object (.-parentElement element)
                        clone-style $ browser-object (.-style el')
                      .!appendChild parent el'
                      js/setTimeout
                        fn ()
                          -> clone-style .-opacity $ set! 0.01
                          -> clone-style .-transform $ set! "|scale(0.8) translate(-24px,0)"
                          -> clone-style .-width $ set! 0
                        , 10
                      js/setTimeout
                        fn () $ .!remove el'
                        , 300
              ...
              match directive
                ...
                (:links links)
                  div
                    ...
                    , & $ -> links
                      map $ fn (xs)
                        if (enum? xs) (render-content xs on-open)
                          <> $ str "|Unknown " xs
                (:route target title)
                  div
